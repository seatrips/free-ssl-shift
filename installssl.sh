...
echo "* If you don't accept the following question this script will restore your file to the above version."
	...
	if [[  $REPLY =~ ^[Yy]$ ]]
	   then
		...
		echo "cd /home/$SSLUSER/free-ssl-shift/" >> start_renew.sh
		echo "source config.sh" >> start_renew.sh
		echo "bash renewssl.sh" >> start_renew.sh
...
echo
echo "Go to your Shift config.json file and edit ssl section like the following:" | tee -a $LOG
echo "    \"ssl\": {" | tee -a $LOG
...
echo "* 12 * * WED bash /home/$SSLUSER/free-ssl-shift/start_renew.sh >> /home/$SSLUSER/free-ssl-shift/logs/cron.log" | tee -a $LOG
echo " " | tee -a $LOG
